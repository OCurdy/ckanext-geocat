...
def map_contact_points(geocat_contact_point):
    contacts = [{"name": geocat_contact_point, "email": geocat_contact_point}]
    return contacts
...
def map_qualified_relations(
    geocat_qualified_relations, organization_slug, valid_identifiers
):
    ...
    if name and normed_protocol == xpath_utils.MAP_PROTOCOL:
        return {
            ...
            "en": (
                MAP_PROTOCOL_PREFIX
                + " "
                + _remove_duplicate_term_in_name(name["en"], "Preview")
            ).strip(),
